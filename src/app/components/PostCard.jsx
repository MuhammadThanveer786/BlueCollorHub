...
export default function PostCard({ post }) {
  ...
  useEffect(() => {
    ...
    // Check session user data (assuming it's populated with full user)
  ...
  return (
    <div className="max-w-2xl mx-auto my-6 border border-gray-200 shadow-md bg-white rounded-lg overflow-hidden">
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        <Link href={`/dashboard/profile/${post.userId?._id}`} legacyBehavior>
             <a className="flex items-center gap-3 group min-w-0">
        ...
        <div ref={commentsRef} className="border-t border-gray-200 p-4 bg-gray-50">
          <div className="flex gap-2 mb-4">
            ...
            <input
              ...
              value={newComment}
              onChange={(e) => setNewComment(e.Ttarget.value)}
          ...
          <div className={`space-y-3 ${showComments === "expanded" ? "max-h-64 overflow-auto pr-2" : ""}`}>
            {(comments && comments.length > 0) ? (
                 (showComments === "expanded" ? comments : comments.slice(0, 2)).map((c, idx) => {
                   ...
                   const userPic = user.profilePic || c.profilePic || c.avatar || "/profile.jpg";

                   return (
                     <div key={commentId} className="flex items-start gap-2">
                       <Link href={`/dashboard/profile/${user._id || c.userId}`} legacyBehavior>
                           <a>
                       ...
                       <div className="bg-gray-100 p-2 rounded-lg w-full text-sm">
                         <Link href={`/dashboard/profile/${user._id || c.userId}`} legacyBehavior>
                              <a className="font-semibold text-xs hover:underline">{userName}</a>
                         </Link>
                 ...
                 feedbacks.map((fb, idx) => {
                     ...
                     return (
                         <div key={fb._id || `feedback-${idx}`} className="flex items-start gap-2 text-sm">
                             <Link href={`/dashboard/profile/${ratingUserId._id || fb.userId}`} legacyBehavior>
                                 <a><img src={ratingUserPic} alt={ratingUserName} className="w-7 h-7 rounded-full object-cover bg-gray-200 flex-shrink-0"/></a>
                             </Link>
                             <div className="flex-1">
                                 <div className="flex items-center gap-1">
                                     <Link href={`/dashboard/profile/${ratingUserId._id || fb.userId}`} legacyBehavior>
                                          <a className="font-semibold text-xs hover:underline">{ratingUserName}</a>
                                     </Link>
                                     <div className="flex">
                                          {Array.from({ length: 5 }, (_, i) => (
