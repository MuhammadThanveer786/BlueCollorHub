// src/app/dashboard/posts/page.js
...
// This is a new, small component to render just the grid item
function PostGridItem({ post }) {
  return (
    // **FIXED CODE:** Removed legacyBehavior and the <a> tag. 
    // The className is moved directly to the Link component.
    <Link 
...
export default function PostPage() {
    ...
    const fetchPosts = async () => {
      ...
      try {
        const { data } = await axios.get("/api/post/search", {
          params: { query, location, category, skill },
        });
  ...
  if (posts.length === 0) {
    return <div className="text-center py-10 text-gray-500">No posts found matching your criteria.</div>;
  }
